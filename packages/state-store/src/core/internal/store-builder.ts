...
import { createStoreInternal } from './create-store-internal';
import { ServerStoreConfigBuilder } from './server-store-builder';
import { applyMiddlewares, connectDevTools } from './store-enhancers';
...
export class StoreBuilder<TState extends Record<string, NonNullable<unknown>>> {
  ...
  initialState(
    ...
    return isServer
      ? new ServerStoreConfigBuilder<TState>({ initialState })
      : new ClientStoreConfigBuilder<
          TState,
          NonNullable<unknown>,
          NonNullable<unknown>,
          NonNullable<unknown>
