...
import type { Action, ActionsApi } from '../types/internal/action';
import type { StoreInternal } from '../types/internal/store';
  ...
  private createActionWrapper<K extends keyof TActions>(key: string, actionFn: TActions[K]) {
    return (
      ...
      const stateUpdatePayload =
        typeof result === 'function' ? result(this.store.getState()) : result;
