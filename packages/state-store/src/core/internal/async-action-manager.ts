...
import type { AsyncState } from '../types/internal/state';
import type { AsyncActionsDef, AsyncResult } from '../types/public-types';
...
> {
   ...
   */
  private createAsyncActionWrapper<K extends keyof TAsyncActions>(
    key: K,
    asyncActionFn: TAsyncActions[K],
  ) {
    // eslint-disable-next-line sonarjs/cognitive-complexity
    return async (...args: Parameters<TAsyncActions[K]>): Promise<AsyncResult<Partial<TState>>> => {
      // 각 요청마다 고유 ID 생성
      const requestId = `${String(key)}_${Date.now()}_${Math.random().toString(36).slice(2, 9)}`;
        ...
        const timeoutId = setTimeout(() => {
          ...
          if (this.asyncStateMap[key].pending && this.activeActionRequests.get(key) === requestId) {
            this.abortControllers
              .get(requestId)
              ?.abort(new Error('Request timed out after 1 minute'));
   ...
   * 비동기 액션 상태를 초기화합니다.
   */
  private initializeAsyncState(
    asyncActions: TAsyncActions,
  ): Record<keyof TAsyncActions, AsyncState> {
