...
import type { ComputedDef } from '../../types/public-types';

import type { IComputedManager } from './interfaces';
...
{
  private readonly computed: TComputed | undefined;
      ...
      fx(entries(this.computed))
        ...
        .each(([key, computedFn]) => {
          ...
          try {
            ...
            if (
              isDevelopment && // 일반적인 undefined 프로퍼티 에러는 조용히 처리
  ...
  private topologicalSort(keys: Array<keyof TComputed>): Array<keyof TComputed> {
    ...
    const visit = (key: keyof TComputed) => {
      ...
      if (tempVisited.has(key)) {
        ...
        if (cycleStart >= 0) {
          ...
          if (isDevelopment) {
            const cycle = [...pathArray.slice(cycleStart), key];
            console.warn(
    ...
    if (cyclicDependencies.size > 0 && isDevelopment) {
      console.warn(
        `[ComputedManager] Some computed values with circular dependencies will not update correctly: ${[
          ...cyclicDependencies,
        ]
          .map(String)
          .join(', ')}`,
  ...
  updateComputedValues(changedStateKeys: Set<string>, currentState: TState): void {
    ...
    const sortedComputedKeys = this.allComputedKeysInOrder
      ? computedKeysToUpdate.size === this.allComputedKeysInOrder.length
        ? this.allComputedKeysInOrder
        : this.allComputedKeysInOrder.filter(key => computedKeysToUpdate.has(key))
      : this.topologicalSort([...computedKeysToUpdate]);
    ...
    for (const key of sortedComputedKeys) {
      ...
      const depsChanged =
        ...
        [...dependencies].some(dep => !oldDeps.has(dep));
      ...
      if (
        prevValue !== newValue && // 깊은 비교는 비용이 크므로 필요한 경우에만 수행
        !deepEqual(prevValue, newValue)
      ) {
  ...
  getDebugInfo() {
    return {
      ...
      keys: [...this.computedCache.keys()].map(String),
      dependencyGraph: Object.fromEntries(
        [...this.computedDependencies.entries()].map(([key, deps]) => [String(key), [...deps]]),
      ),
    };
  }
}
