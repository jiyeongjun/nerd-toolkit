...
import { SubscriptionTree } from '../../../utils/subscription-tree';

import type { ISubscriptionManager } from './interfaces';
...
export class SubscriptionManager implements ISubscriptionManager {
  ...
  subscribeState<TState extends Record<string, any>, T>(
    ...
    // 구독 트리에 직접 등록
    const unsubscribe = this.subscriptionTree.subscribe(
      subscriberId,
      stateListener,
  ...
  subscribeStates<TState extends Record<string, any>, S extends unknown[]>(
    ...
    if (!getState || !trackDependencies) {
      throw new Error(
        '[SubscriptionManager] Required parameters missing: getState or trackDependencies',
      );
    }
