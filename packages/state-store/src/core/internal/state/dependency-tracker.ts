...
import type { IDependencyTracker } from './interfaces';
 ...
 */
export class DependencyTracker<TState extends Record<string, any>>
  implements IDependencyTracker<TState>
{
  // 셀렉터 의존성
  private selectorDependencies: WeakMap<(state: Readonly<TState>) => unknown, Set<string>> =
    new WeakMap();
  ...
  private isSafeToProxy(obj: any): boolean {
    ...
    return !(objProto !== Object.prototype && objProto !== Array.prototype);
  }
  ...
  createTrackingProxy(
    ...
    if (proxyTarget.__isProxyTracker && proxyTarget.__dependenciesRef === dependencies) {
      return target;
    }
    ...
    return new Proxy(target, {
      get: (obj, prop) => {
        ...
        // 현재 경로 생성 및 의존성 추가
        const currentPath = path ? (/^\d+$/.test(key) ? `${path}[${key}]` : `${path}.${key}`) : key;
