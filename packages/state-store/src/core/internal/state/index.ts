import { createBatchedUpdates } from '../../../utils/create-batched-updates';
import { safeAction } from '../../../utils/error-boundary';
import { createSelector } from '../../../utils/selector-memoization';
import type { ComputedDef } from '../../types/public-types';

import { ComputedManager } from './computed-manager';
import { DependencyTracker } from './dependency-tracker';
import type { UpdateOptions } from './interfaces';
  ...
  _setState(newState: Partial<TState>, options: UpdateOptions = {}): void {
    ...
    return safeAction(
      ...
      () => {
        ...
        // 상태 업데이트 실행
        const { changedKeys, changedPaths, hasStructuralChange } = this.stateContainer.updateState(
          newState,
          {
            statePath,
          },
        ...
        if (!silent) {
          this.batchedUpdates.scheduleUpdate(
            () => {
              // 구독 트리를 통한 효율적인 알림
              this.subscriptionManager.notifySubscribers([...changedPaths]);
            },
            {
