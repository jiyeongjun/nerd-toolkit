...
import type { Action } from '../types/internal/action';
import type { Middleware, StoreInternal } from '../types/internal/store';
import type { ActionsDef, AsyncActionsDef, ComputedDef } from '../types/public-types';
...
>(
  ...
  try {
    ...
    // isTimeTraveling 상태 관리를 위한 헬퍼 함수
    const withTimeTravelProtection = (action: () => void) => {
      const previousTimeTravelingState = isTimeTraveling;
      try {
        isTimeTraveling = true;
        action();
      } catch {
        ...
        isTimeTraveling = previousTimeTravelingState;
      }
    };

    // JUMP_TO_STATE 처리 함수
    const handleJumpToState = (stateStr: string) => {
      withTimeTravelProtection(() => {
        let newState;
        try {
          newState = deserializeState(stateStr);
        } catch (parseError) {
          // 구체적인 에러를 던져서 상위에서 잡거나, 여기서 처리할 수 있습니다.
    ...
    // JUMP_TO_ACTION 처리 함수
    const handleJumpToAction = (payload: any, stateStr?: string) => {
      withTimeTravelProtection(() => {
        const actionId = Number.parseInt(payload.actionId, 10);
        if (!Number.isNaN(actionId)) {
          jumpToAction(actionId);
        } else if (stateStr) {
          const newState = deserializeState(stateStr);
          originalSetState(newState);
        }
      });
    };

    // DISPATCH (DevTools에서 발생한) 처리 함수
    const handleDevToolsDispatch = (actionStr: string) => {
      try {
        const action = JSON.parse(actionStr);
        originalDispatch(action); // 이 originalDispatch는 isTimeTraveling 로직이 필요 없을 수 있습니다.
      } catch {
        /* empty */
      }
    };
    ...
    devTools.subscribe((message: any) => {
      // 가드 클로즈: DISPATCH 타입이 아니거나 payload가 없으면 바로 종료
      if (message.type !== 'DISPATCH' || !message.payload) {
        return;
      }

      const { payload, state } = message; // state는 message.state
      ...
      switch (payload.type) {
        ...
        case 'JUMP_TO_ACTION': {
          // handleJumpToAction은 state가 없을 수도 있음을 내부적으로 처리
          handleJumpToAction(payload, state);
          break;
        }
        case 'DISPATCH': {
          // DevTools에서 발생한 DISPATCH
          if (payload.action) {
            // action 문자열이 있는지 확인
            handleDevToolsDispatch(payload.action);
          }
          break;
        }
        default: {
          // 알려지지 않은 payload 타입 처리 (옵션)
          break;
        }
      }
    });
    ...
    if (isDevelopment) {
      ...
      const storeWithDevTools = store as StoreWithDevTools<
        TState,
        TComputed,
        TActions,
        TAsyncActions
