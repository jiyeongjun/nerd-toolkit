...
export interface Subscriber {
  ...
  throttle?: number | undefined; // 스로틀링 시간 (밀리초)
  lastExecuted?: number | undefined; // 마지막 실행 시간
...
export class SubscriptionTree {
  ...
  private isPathPrefixMatch(prefix: string[], path: string[]): boolean {
    ...
    for (const [i, element] of prefix.entries()) {
      if (element !== path[i]) return false;
    }
  ...
  private parsePath(path: string): string[] {
    ...
    const normalizedPath = path.replaceAll(/\[(\d+)]/g, '.$1');
  ...
  notifySubscribers(changedPaths: string[]): void {
    ...
    try {
      ...
      for (const path of changedPaths) {
        ...
        const rootSubscribers = [...this.root.subscribers.values()];
        ...
        for (const [subscriberId, wildcardInfo] of this.wildcardSubscriberMap.entries()) {
          ...
          if (type === SINGLE_LEVEL_WILDCARD) {
            ...
            type === MULTI_LEVEL_WILDCARD && // 다중 레벨 와일드카드의 경우, 부모 경로가 현재 경로의 접두사인지 확인
        ...
        for (const segment of segments) {
          ...
          if (currentNode.singleLevelWildcardNode) {
            const wildcardSubscribers = [
              ...currentNode.singleLevelWildcardNode.subscribers.values(),
            ];
          ...
          if (currentNode.multiLevelWildcardNode) {
            const multiWildcardSubscribers = [
          ...
          // 현재 노드 구독자 추가 (스냅샷으로)
          const currentSubscribers = [...currentNode.subscribers.values()];
      ...
      // 우선순위에 따라 구독자 정렬
      const sortedSubscribers = [...subscribersToNotify.values()].sort(
