...
export function createBatchedUpdates() {
  ...
  const scheduleFlush = (): void => {
    ...
    if (hasHighPriorityUpdates) {
      ...
      state.rafId = hasRequestAnimationFrame
        ? requestAnimationFrame(flushUpdates)
        : (setTimeout(flushUpdates, 16) as unknown as number); // 약 60fps에 해당하는 시간
    }
  };
  ...
  const isDuplicateUpdate = (id: string): boolean => {
    ...
    return [state.pendingUpdates.high, state.pendingUpdates.normal, state.pendingUpdates.low].some(
      queue => queue.some(update => update.id === id),
    );
  };
