...
import type { VisitedMap } from './object-traversal';
import { createVisitedMap, detectCycle, getObjectType } from './object-traversal';
...
const COMPARE_CACHE_TTL = 60_000; // 캐시 유효 시간 (1분)
    ...
    switch (objTypeA.type) {
      ...
      case 'set': {
        ...
        if (allPrimitives) {
          ...
          const stringSetB = new Set([...setB].map(String));
          result = [...setA].every(item => stringSetB.has(String(item)));
        } else {
          const bValues = [...setB];
          ...
          for (const itemA of setA) {
            ...
            if (itemA === null || typeof itemA !== 'object') {
              for (const [i, bValue] of bValues.entries()) {
              ...
              for (const [i, itemB] of bValues.entries()) {
                if (matchedIndices.has(i)) continue;
        ...
        if (arrA.length === arrB.length) {
          ...
          if (arrA.length === 0) {
            ...
            for (const [i, element] of arrA.entries()) {
              if (!deepEqual(element, arrB[i], depth + 1, visited)) {
                result = false;
                break;
              }
            }
            ...
            // 일반 배열 비교
            for (const [i, element] of arrA.entries()) {
              if (element === arrB[i]) continue; // 참조 같으면 스킵
              if (!deepEqual(element, arrB[i], depth + 1, visited)) {
                result = false;
                break;
              }
...
function hasStructuralDifferences(current: unknown, next: unknown): boolean {
  ...
  switch (currentType.type) {
    ...
    default: {
      ...
      return false;
    } // 이미 타입 체크를 통과했으므로 구조적으로는 같다고 간주
  }
}
